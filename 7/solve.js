...
const solve2 = (input) => {
  ...
  input.forEach(rule => {
    ...
    if (rule.includes("no other bags")) {
      ...
      const numberColor = rule.slice(rule.indexOf('contain') + 8, rule.indexOf('.'));
      ...
      numberColors[numberColors.length - 1] = numberColors[numberColors.length - 1].slice(0, -1);
...
const getCountContainedFromColor = (color, rules) => {
  if (rules[color].length === 0) {
    ...
    return rules[color].reduce((acc, cur) => {
      const res = acc + cur[0] + cur[0] * getCountContainedFromColor(cur[1], rules);
      return res;
    }, 0);
  }
}
...
const getTupleFromNumberColor = (numberColor) => {
  ...
  const color = numberColor.slice(numberColor.indexOf(' ') + 1, numberColor.indexOf('bag') - 1);
  return [number, color];
}
